...
local function loop()
    ...
    if is_power_range and not is_pitmode then
...
local function init()
	...
	if not is_add_param_table then
		...
		assert(param:add_table(TABLE_KEY, TABLE_PREFIX, 14), "The parameter table wasn`t created")
		...
		param:add_param(TABLE_KEY, 4, PARAMS.POWER_RC, 0)
		param:add_param(TABLE_KEY, 5, PARAMS.MIN_POWER, 25)
		param:add_param(TABLE_KEY, 6, PARAMS.TRIM_POWER, 25)
		param:add_param(TABLE_KEY, 7, PARAMS.FREQS[1], 0)
		param:add_param(TABLE_KEY, 8, PARAMS.FREQS[2], 0)
		param:add_param(TABLE_KEY, 9, PARAMS.FREQS[3], 0)
		param:add_param(TABLE_KEY, 10, PARAMS.FREQS[4], 0)
		param:add_param(TABLE_KEY, 11, PARAMS.FREQS[5], 0)
		param:add_param(TABLE_KEY, 12, PARAMS.FREQS[6], 0)
		param:add_param(TABLE_KEY, 13, PARAMS.FREQS[7], 0)
		param:add_param(TABLE_KEY, 14, PARAMS.FREQS[8], 0)
    ...
    if is_enable and not is_init_PWR_RC_channel then
        ...
        if PWR_RC_channel ~= nil or PWR_RC_channel > 5 then
            gcs:send_text(6, string.format("4 : Set POWER RC channel %d", PWR_RC_channel))
    ...
    if is_init_PWR_RC_channel and not is_power_range then
        ...
        for i = 1, 3 do
            lower = 990 + (i - 1) * range_step
            upper = lower + range_step
	...
	if (is_enable and not is_init) and not is_init_PM_RC_channel then
		...
		if PM_RC_channel ~= nil or PM_RC_channel > 5 then
			gcs:send_text(6, "6 : Set PITMODE RC channel ... " .. PM_RC_channel)
			gcs:send_text(6, "6 : Done!\n ")
			is_init_PM_RC_channel = true
			return init, 100
		end

		gcs:send_text(6, "6 : VTX PITMODE RC channel wasn`t defined!\n ")
	end

    -- init FRECS
	if is_init_RC_channel and not is_init_freq then
		gcs:send_text(6, "7: Set RC range ... ")
		...
		gcs:send_text(6, "7 : CHANNEL FREQS wasn`t defined...\n ")
	end

    -- set CHANNEL RC range
	if is_init_freq and not is_init_boundaries then
		gcs:send_text(6, "8 : Create boundaries table ... \n ")
		gcs:send_text(6, " - - - - - - - - - - - - - - - -")
        local step = math.floor(RC_RANGE / #frequencies)
		for i, freq in ipairs(frequencies) do
		...
		gcs:send_text(6, " - - - - - - - - - - - - - - - -\n ")
		gcs:send_text(6, "8 : Done!\n ")
		is_init_boundaries = true
		return init, 100
	end

	if (is_init_boundaries or is_init_PM_RC_channel or is_power_range) and not is_init then
		is_init = true
		gcs:send_text(6, string.format("Initialize CHANNEL FREQS: %q", is_init_boundaries))
		gcs:send_text(6, string.format("Initialize VTX POWER control: %q", is_power_range))
